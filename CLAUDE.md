...
- **아트 파이프라인 (유저 확정)**: 최종 이미지는 전부 **PixelLab MCP로 생성** — 별도 아트 세션(A트랙, `DevPlan/README.md` 참조)에서. 개발 세션은 플레이스홀더(색 사각형 + 이모지/텍스트)로 진행하되 **아트 교체 가능 구조 필수**: 스프라이트는 SO 필드/SpriteRenderer 참조로만 연결(코드 하드코딩 금지), 임포트 규약(PPU·필터·압축)은 A01에서 확정. PixelLab은 크레딧 소모 — **A01 스타일 가이드 유저 승인 전 대량 생성 금지**
